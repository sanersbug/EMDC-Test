...
def train_emdc(args, n_epoch, model, train_loader, optimizer, ema, scheduler, criterion1, criterion2, criterion3, criterion4, n_iters_every_epoch, train_set, writer):
    # model training
    model.train()
    total_loss = 0.
    epoch_iterator = tqdm(train_loader, bar_format="{l_bar}|[{elapsed}]", dynamic_ncols=True,
        ...
        if (i + 1) % 5 == 0 or (i + 1) == n_iters_every_epoch:
            epoch_iterator.set_description(
                'Training epoch {}: {} / {}, training Loss {loss:.8f}\t lr: {lr:.5f}'.format(
                    n_epoch, i + 1, n_iters_every_epoch, loss=total_loss / len(train_set),
    ...
    if args.c2f:
        writer.add_scalar('train/lr_pretrained_trunk', scalar_value=optimizer.param_groups[-2]['lr'],
                          global_step=n_epoch)



def test_emdc(n_epoch, model, ema, val_loader, criterion1, test_set, writer):
    metric = Metric()
    ...
    with ema.average_parameters():
        ...
        with torch.no_grad():
            ...
            print(
                'Epoch {} Validation Loss: {:.3f}, score:{:.4f}, time:{:.2f}ms per frame'.format(n_epoch, average_loss,
                                                                                                 metric_score,
                                                                                                 average_time))
...
def train_cspn(args, n_epoch, model, train_loader, optimizer, ema, scheduler, criterion1, criterion2, criterion3, criterion4, n_iters_every_epoch, train_set, writer):
    # model training
    model.train()
    total_loss = 0.
    epoch_iterator = tqdm(train_loader, bar_format="{l_bar}|[{elapsed}]", dynamic_ncols=True,
    ...
    for i, batch_x in enumerate(epoch_iterator):
        ...
        # forward
        rgbd_image = torch.cat((input, dep), 1)
        output = model(rgbd_image)
        ...
        if 'criterion4' in args.loss:
            loss4 = criterion4(output, target)
            loss += 0.7 * loss4 / (loss4 / loss2).detach()  # default 0.2
...
def test_cspn(n_epoch, model, ema, val_loader, criterion1, test_set, writer):
    metric = Metric()
...
def train_fusion(args, n_epoch, model, train_loader, optimizer, ema, scheduler, criterion1, criterion2, criterion3, criterion4, n_iters_every_epoch, train_set, writer):
    # model training
    model.train()
    total_loss = 0.
    epoch_iterator = tqdm(train_loader, bar_format="{l_bar}|[{elapsed}]", dynamic_ncols=True,
...
def test_fusion(n_epoch, model, ema, val_loader, criterion1, test_set, writer):
    metric = Metric()
...
def main(args):
    ...
    train_set = get_training_set(data_root=args.data_root, datatype=args.datatype, model_name=args.arch)
    test_set = get_test_set(data_root=args.data_root, datatype=args.datatype, model_name=args.arch)
    ...
    for n_epoch in range(args.num_epochs):
        if args.arch == 'EMDC':
            train_emdc(args, n_epoch, model, train_loader, optimizer, ema, scheduler, criterion1, criterion2, criterion3, criterion4, n_iters_every_epoch, train_set, writer)
        elif args.arch == 'CSPN':
            train_cspn(args, n_epoch, model, train_loader, optimizer, ema, scheduler, criterion1, criterion2,
                       criterion3, criterion4, n_iters_every_epoch, train_set, writer)
        with ema.average_parameters():
            if args.arch == 'EMDC':
                metric_score = test_emdc(n_epoch, model, ema, val_loader, criterion1, test_set, writer)
            elif args.arch == 'CSPN':
                metric_score = test_cspn(n_epoch, model, ema, val_loader, criterion1, test_set, writer)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--arch", default="CSPN", help="Decide the model structure")
    parser.add_argument("--name", default="cspn", help="Identifier of this train mission.")
    ...
    parser.add_argument("--num_epochs", default=200, type=int, help="Total number of training epochs to perform.")
    ...
    parser.add_argument("--num_warmup_epochs", default=10, type=int, help="Steps of training to perform learning rate warmup for.")
    parser.add_argument("--gpu", default='0', type=str, help="Select single GPU idx.")
    
    # loss function
    parser.add_argument("--loss", default=["criterion2", "criterion4"], action='append', help="losses used")

    # add
    parser.add_argument("--data_root", default="./data/MIPI2/", type=str,
                        help="The output directory of data root, the root contain train and val file.")
    parser.add_argument("--datatype", default="mipi", help="The name of public data set, include [mipi, nyu]")
