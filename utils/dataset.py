...
def get_training_set(data_root='', datatype='mipi', model_name=''):
    train_txt = data_root + 'data_train.list'
    return mipi_dataset(train_txt, 'train', data_root, datatype, model_name)


def get_test_set(data_root='', datatype='mipi', model_name=''):
    test_txt = data_root + 'data_val.list'
    return mipi_dataset(test_txt, 'test', data_root, datatype, model_name)

# MIPI Dataset
class mipi_dataset(data.Dataset):
    def __init__(self, txt_path, flag, data_root, datatype, model_name):
        ...
        if self.flag != 'test':
            ...
            if self.datatype == 'mipi' and self.model_name != 'CSPN':
                with open(txt_path, 'r') as fh:
                    pairs = []
                    for line in fh:
                        line = line.rstrip()
                    ...
                    self.pairs = pairs
            elif self.datatype == 'mipi' and self.model_name == 'CSPN':
                with open(txt_path, 'r') as fh:
                    pairs = []
                    for line in fh:
                        line = line.rstrip()
        ...
        if self.model_name == 'EMDC':
            crop_size = (256, 256)
        elif self.model_name == 'CSPN':
            crop_size = (228, 304)
    ...
    def __getitem__(self, index):
        ...
        if self.flag == 'train':
            ...
            if self.datatype == 'mipi' and self.model_name != 'CSPN':
                ...
                rgb = rgb[:, top:(top + self.crop_size[0]), left:(left + self.crop_size[1])]
                dep = dep[:, top:(top + self.crop_size[0]), left:(left + self.crop_size[1])]
                # dep_sp = dep_sp[:, top:(top + self.crop_size[0]), left:(left + self.crop_size[1])]

                if self.grid_spot:
                    dep_sp = self.get_sparse_depth_grid(dep)
                else:
                    dep_sp = self.get_sparse_depth(dep, self.num_spot)
    ...
    def _load_png(self, index):
        if self.flag != 'test':
            if self.datatype == 'mipi':
                ...
                input = ops.join(self.data_root, 'train', input)
                target = ops.join(self.data_root, 'train', target)

                np_in = cv2.imread(input)
                np_in = cv2.cvtColor(np_in, cv2.COLOR_BGR2RGB)
                np_tar = cv2.imread(target, cv2.IMREAD_ANYDEPTH)
            elif self.datatype == 'nyu':
                h5_path_ = self.pairs[index]
                h5_path = os.path.join(self.data_root, 'train', h5_path_)
                input, target = h5_loader(h5_path)
                np_in = cv2.cvtColor(input, cv2.COLOR_BGR2RGB)
                np_tar = target
            ...
            if self.datatype == 'mipi' and self.model_name != 'CSPN':
                ...
                input = ops.join(self.data_root, 'test', input)
                dep_sp = ops.join(self.data_root, 'test', dep_sp)
                target = ops.join(self.data_root, 'test', target)
                ...
                h5_path_ = self.pairs[index]
                h5_path = os.path.join(self.data_root, 'val', h5_path_)
                input, target = h5_loader(h5_path)
                np_in = cv2.cvtColor(input, cv2.COLOR_BGR2RGB)
                # np_tar0 = torch.tensor(np.expand_dims(target, 0))
                # np_sp = self.get_sparse_depth(np_tar0, self.num_spot)
                np_tar = target
                return np_in, np_tar
        ...
        if self.cutmix and self.flag == 'train':
            if self.datatype == 'mipi':
                ...
                input_ = ops.join(self.data_root, 'train', input_)
                target_ = ops.join(self.data_root, 'train', target_)
                ...
                idx = random.randint(0, self.length - 1)
                h5_path_ = self.pairs[idx]
                h5_path = os.path.join(self.data_root, 'train', h5_path_)
                input, target = h5_loader(h5_path)
                np_in_ = cv2.cvtColor(input, cv2.COLOR_BGR2RGB)
                np_tar_ = target
            ...
            np_in[bby1 // scale: bby2 // scale, bbx1 // scale: bbx2 // scale, :] = np_in_[
                                                                                   bby1 // scale: bby2 // scale,
                                                                                   bbx1 // scale: bbx2 // scale, :]
    ...
    def get_sparse_depth(self, dep, num_spot):
        ...
        idx_nnz = torch.nonzero(dep.contiguous().view(-1) > 0.0001)
